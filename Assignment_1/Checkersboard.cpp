/*
	...
	Circles.cpp: render checkers board using pixel shader
*/
...
const char *pixelShader = R"(
	...
	void main() {
		...
		bool x = Odd(gl_FragCoord.x / 50);
		bool y = Odd(gl_FragCoord.y / 50);  

		if(x == y) {
			pColor = vec4(0, 0, 0, 1);							// r, g, b, alpha
		}
		else if(gl_FragCoord.y < 200) {
			pColor = vec4(1, 1, 1, 1);
		}
		else {
			pColor = vec4(1, 0, 0, 1);
...
int main() {												// application entry
	...
	GLFWwindow *w = glfwCreateWindow(400, 400, "Checkersboard", NULL, NULL);
	if (!w)
