/*
	...
	Circles.cpp: render concentric circles using pixel shader
*/
...
const char *pixelShader = R"(
	...
	void main() {
		...
		float ringThickness = 10;
		vec2 center = vec2(200);
		float dist = distance(gl_FragCoord.xy, center.xy);
		pColor = vec4(sin(dist / ringThickness), sin((dist - 200) / ringThickness), sin((dist - 400) / ringThickness), 1);
	}
)";
...
int main() {												// application entry
	...
	GLFWwindow *w = glfwCreateWindow(400, 400, "Circles", NULL, NULL);
	if (!w)
