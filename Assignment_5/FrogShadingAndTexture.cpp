/*
    ...
    FrogShadingAndTexture.cpp: read and draw an .obj file, render it three times:
        1 - render solid obj with Phong shading
        2 - render solid obj with Phong shading and texture applied
        3 - render solid obj with Phong shading and texture applied with twice the frequency
*/
...
const char* filename = "./Chessbd.tga";
...
int textureUnit = 0;  // user GPU buffer
GLuint textureName = 0; // default, assigned later

// Camera
int winW = 1200, winH = 400;
float aspectRatio = (float)(winW / 3) / (float)(winH);
...
const char *pixelShader = R"(
    ...
    out vec4 pColor;
    uniform float a = .2;
    uniform vec3 lightPos = vec3(1,0,0); // 3D light location w/ default
    ...
    void main() {
        ...
        vec3 color = isTextured == 1 ? texture(textureImage, vuv).rgb : vec3(.7, 1, 0);
        if(isTextured == 2)
            color = texture(textureImage, vuv * 2).rgb;
...
void Resize(GLFWwindow* window, int width, int height) {
    camera.Resize(winW = width / 3, winH = height);
...
void Display(GLFWwindow *w) {
    ...
    // draw shaded frog on the left
    SetUniform(program, "isTextured", 0);
    glViewport(0, 0, winW / 3, winH);
    glDrawElements(GL_TRIANGLES, 3 * triangles.size(), GL_UNSIGNED_INT, &triangles[0]);

    // draw textured frog in the middle
    SetUniform(program, "isTextured", 1);
    glViewport(winW / 3, 0, winW / 3, winH);
    glDrawElements(GL_TRIANGLES, 3 * triangles.size(), GL_UNSIGNED_INT, &triangles[0]);

    // draw textured frog *2 on the right
    SetUniform(program, "isTextured", 2);
    glViewport(2 * winW / 3, 0, winW / 3, winH);
    glDrawElements(GL_TRIANGLES, 3 * triangles.size(), GL_UNSIGNED_INT, &triangles[0]);
...
int main(int argc, char **argv) {
    ...
    GLFWwindow* w = glfwCreateWindow(winW, winH, "Frog Shading and Texture", NULL, NULL);
    if (!w) {
        glfwTerminate();
        return 1;
    }
