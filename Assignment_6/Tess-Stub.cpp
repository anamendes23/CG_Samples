/*
	...
	Tess-Stub.cpp - stub application using tessellation shader
*/
...
const char *textureFilename = "./Chessbd.tga";
...
const char *teShaderCode = R"(
	...
	vec3 PtFromSphere(float u, float v) {
		...
		//float eFactor = cos(elevation + time); // cool transition hybrid sphere and cylinder
		//float eFactor = cos(elevation) + cos(time); // cool reverse cylinder to opened sphere
		float a = (1 + cos(time)) / 2; // sinusoidal function between 0 and 1
		float eFactor = 1 + a * (cos(elevation) - 1); // sinusoidal function between 1 and cos(elevation)
	...
	void main() {
		...
		float den = sqrt(p.x * p.x + p.z * p.z);
		vec3 n = normalize(vec3(p.x / den, 0, p.z / den));
		//vec3 n = p;
